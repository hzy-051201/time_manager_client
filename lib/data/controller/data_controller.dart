...
import 'package:time_manager_client/data/repository/network_weather.dart';
...
class DataController extends GetxController {
  ...
  WeatherInfo? get weatherInfo => _weatherInfo.value;
  String? get weatherSuggestion => _weatherSuggestion.value;
  ...
  void _loadWeatherFromCache() {
    try {
      final cached = Box.simpleData.read<String>('weather_data');
      if (cached != null) {
        ...
        if (expireAt != null && DateTime.now().millisecondsSinceEpoch < expireAt) {
          ...
          _weatherInfo.value = WeatherInfo(
            ...
            temp: weatherData['temp'].toDouble(),
            feelsLike: weatherData['feelsLike'].toDouble(),
            tempMin: weatherData['tempMin'].toDouble(),
            tempMax: weatherData['tempMax'].toDouble(),
            humidity: weatherData['humidity'],
            windSpeed: weatherData['windSpeed'].toDouble(),
            city: weatherData['city'],
            timestamp: weatherData['timestamp'],
          );
          _weatherSuggestion.value = data['suggestion'];
          _weatherCity = data['city'];
        } else {
          // 缓存过期，清除
          Box.simpleData.remove('weather_data');
        }
      }
    } catch (e) {
      logger.e('加载天气缓存失败: $e');
  ...
  Future<void> refreshWeather() async {
    await initWeather(_weatherCity);
  }

  /// 获取天气建议（结合任务）
  Future<String?> getWeatherSuggestionWithTasks() async {
...
- 温度：${weather.temp.toStringAsFixed(1)}℃（体感${weather.feelsLike.toStringAsFixed(1)}℃）
- 湿度：${weather.humidity}%
...
${travelTasks.isNotEmpty ? '特别关注：用户有出差/出行类任务，请重点考虑天气对出行的影响。' : '请关注天气对户外活动的影响。'}''';
    ...
    try {
      final suggestion = await NetworkAi.askAi(
        prompt,
